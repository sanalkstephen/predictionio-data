...
object Storage extends Logging {
  ...
  private def sourcesToClientMeta(
    ...
    info(s"sourcesToClientMeta called ${sourceName}")
  ...
  Option[ClientMeta] = {
    try {
      ...
      info(s"Source Name: ${keyedPath} Source Type :  ${sourceType}")
      ...
      val clientConfig = StorageClientConfig(
        ...
        parallel = parallel,
        test = test)
      info(s"clientConfig :  ${clientConfig}")
